...
export default function MediaHomeUploadModal({ isOpen, onClose }) {
	...
	function uploadFiles() {
		...
		axios({
			method: "POST",
			url: selectedMission
				? `/api/missions/${selectedMission.uniqueName}/media_upload`
				: `/api/media`,
	...
	const { data: missions } = useSWR(
		mounted ? `/api/missions/fetch_mission_list_for_media_upload` : null,
		fetcher,
		{
	...
	React.useEffect(() => {
		set_document(document.querySelector("#mediaPage"));
	}, []);
								...
								<Tab.Panels className="mt-2 ">
									<Tab.Panel>
										<Select
											...
											placeholder="Select the mission from which the content was taken (optional)"
