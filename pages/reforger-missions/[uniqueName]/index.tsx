...
function getOutcomeClass(outcomeText: string): string {
  ...
  if (t.includes("civ")) return "text-civ";
  return "";
}
...
function getLeaderString(leader: any) {
...
interface HistoryCardContentProps {
  ...
  index: number;
  totalItems: number;
  missionUniqueName: string;
  missionAuthorID: string;
  session: any;
  sessionRatings: Record<string, string>;
  onRate: (historyEntryId: string, value: string) => void;
  canEditHistory: boolean;
  onEditAAR: () => void;
  onSubmitAAR: (leader: any) => void;
  onViewGmNote: () => void;
}
...
function HistoryCardContent({
  ...
  useEffect(() => {
    if (open && !fetched.current && historyItem.serverSessionId) {
      fetched.current = true;
      axios
        .get(`/api/server-sessions/${historyItem.serverSessionId}`)
        .then((r) => setSessionData(r.data))
        .catch(() => {});
    }
  }, [open, historyItem.serverSessionId]);
  ...
  const canRate =
    ...
    session.user["discord_id"] !== missionAuthorID;

  // Build ApexCharts config from session snapshots
  let chartSeries: any[] | null = null;
  let chartOptions: any | null = null;
  if (sessionData?.snapshots?.length > 0) {
    const startMs = new Date(sessionData.snapshots[0].time).getTime();
    chartSeries = [
      {
        name: "Players",
        data: sessionData.snapshots.map((s: any) => s.players),
      },
    ];
    chartOptions = {
      chart: { type: "line", toolbar: { show: false }, background: "transparent" },
      xaxis: {
        categories: sessionData.snapshots.map((s: any) => {
          const mins = Math.round((new Date(s.time).getTime() - startMs) / 60000);
          return `${mins}m`;
        }),
        tickAmount: 8,
  ...
  return (
    <>
      <Disclosure.Button
        className={`flex flex-row items-center justify-between w-full p-3 mb-2 rounded-lg transition-shadow duration-200 bg-gray-700 hover:bg-gray-600 text-white ${
          open ? "shadow-lg" : "shadow-none"
        }`}
      >
        <div className="flex flex-wrap items-center gap-3 text-left">
          {historyItem.outcome ? (
            <span className={`text-sm font-semibold ${getOutcomeClass(historyItem.outcome)}`}>
              {historyItem.outcome}
            </span>
          ) : (
            <span className="text-sm text-gray-400 italic">No outcome</span>
          )}
          <span className="text-sm text-gray-300">{moment(historyItem.date).format("LL")}</span>
          {historyItem.serverSessionId && (
            <span className="text-xs bg-blue-600/30 text-blue-300 px-1.5 py-0.5 rounded">
              📊 Session
            </span>
          )}
          {historyItem.leaders?.length > 0 && (
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
              {historyItem.leaders.map((leader: any) => (
                <div key={leader.discordID} className="flex items-center gap-1">
                  {leader.displayAvatarURL ? (
                    ...
                    <div className="w-6 h-6 rounded-full bg-gray-500 flex items-center justify-center text-xs text-white flex-shrink-0">
                      {leader.name?.[0]?.toUpperCase() ?? "?"}
                    </div>
                  )}
                  <span className="text-sm text-gray-300">{leader.name}</span>
                  {leader.side && (
                    <span className={`text-xs font-semibold ${getOutcomeClass(leader.side)}`}>
                      {leader.side}
                    </span>
                  )}
                </div>
        ...
        <ChevronRightIcon
          className={`flex-shrink-0 transition-transform duration-150 ease-in-out ${open ? "rotate-90" : ""}`}
          width={22}
          height={22}
          color="white"
        />
      ...
      >
        <Disclosure.Panel static className="mb-4 px-1">
          {/* Player count chart */}
          {chartSeries && chartOptions && (
            <div className="mb-4 rounded-lg overflow-hidden bg-gray-800 p-2">
              <Chart
                  ...
                  return (
                    <button
                      ...
                      className={`btn btn-xs gap-1 ${
                        myRating === v ? "btn-primary" : "btn-ghost dark:text-white"
                      }`}
                      onClick={(e) => {
                        e.stopPropagation();
                        onRate(historyItem._id, v);
                      }}
                    >
                      {emoji} {count}
                    </button>
                ...
                <>
                  <Disclosure.Button
                    className={`flex flex-row items-center justify-between w-full p-2 mb-2 text-white transition-shadow duration-300 bg-gray-600 rounded-lg hover:shadow-lg ${
                      leaderOpen ? "shadow-lg" : "shadow-none"
                    }`}
                  >
                    <div className="flex flex-row items-center">
                      ...
                      <div className="flex flex-row items-end ml-4 text-lg">
                        {!leader.name && (
                          <Shimmer>
                            <div className="rounded-lg" style={{ width: 90, height: 22 }} />
                          </Shimmer>
                        )}
                    ...
                    <ChevronRightIcon
                      className={`duration-150 ease-in-out ${leaderOpen ? "transform rotate-90" : ""}`}
                      width={28}
                      height={28}
                      color="white"
                    />
          ...
          {index + 1 !== totalItems && <hr className="mt-5 mb-3" />}
        </Disclosure.Panel>
      </Transition>
    </>
  );
...
export default function MissionDetails({
  ...
  // Poll server load lock every 10 s — only for GM/Admin/Mission Reviewer
  ...
  return <>
    ...
    >
      <div className="mx-2">
        ...
        {/* TEMPORARY: Mission Review Team has the same access as Arma GM. Remove CREDENTIAL.MISSION_REVIEWER when no longer needed. */}
      {lockState.locked && hasCredsAny(session, [CREDENTIAL.ADMIN, CREDENTIAL.GM, CREDENTIAL.MISSION_REVIEWER]) && (
        ...
        <div className="flex flex-row md:space-x-10">
          ...
          <div className="flex flex-col justify-between flex-1 ">
            ...
            <div>
                        <div className="flex flex-row flex-wrap w-full bg-transparent stats dark:text-white pb-0">
              <div className="m-2 ">
            ...
            {(mission.missionOverview || mission.missionNotes) && (
              <div className="flex gap-2 mt-1 ml-2">
                {mission.missionOverview && (
                  <button
                    className="btn btn-sm btn-info btn-outline"
                    onClick={() => {
                      setSimpleTextViewing(mission.missionOverview);
                      setSimpleTextHeaderViewing("Mission Overview");
                      setSimpleTextModalOpen(true);
                    }}
                  >
                    Mission Overview
                ...
                {mission.missionNotes && (
                  <button
                    className="btn btn-sm btn-info btn-outline"
                    onClick={() => {
                      setSimpleTextViewing(mission.missionNotes);
                      setSimpleTextHeaderViewing("Mission Notes");
                      setSimpleTextModalOpen(true);
                    }}
                  >
                    Mission Notes
        ...
        <div>
          {getHistory() ? (
            getHistory().map((historyItem, index) => (
              <Disclosure key={historyItem._id}>
                {({ open }) => (
                  <HistoryCardContent
                    open={open}
                    historyItem={historyItem}
                    index={index}
                    totalItems={getHistory().length}
                    missionUniqueName={mission.uniqueName}
                    missionAuthorID={mission.authorID}
                    session={session}
                    sessionRatings={sessionRatings}
                    onRate={submitSessionRating}
                    canEditHistory={hasCredsAny(session, [CREDENTIAL.ADMIN, CREDENTIAL.GM, CREDENTIAL.MISSION_REVIEWER])}
                    onEditAAR={() => {
...
export async function getServerSideProps(context) {
  ...
  if (mission["history"]) {
    mission["history"].forEach(history => {
      ...
      if (history["serverSessionId"]) {
        history["serverSessionId"] = history["serverSessionId"].toString();
      }
  ...
  // TEMPORARY: Mission Review Team has the same access as Arma GM until GMs
  // are more familiar with the system. Remove CREDENTIAL.MISSION_REVIEWER when no longer needed.
  if (hasCredsAny(session, [CREDENTIAL.GM, CREDENTIAL.ADMIN, CREDENTIAL.MISSION_REVIEWER])) {
    try {
      const db = (await MyMongo).db("prod");
