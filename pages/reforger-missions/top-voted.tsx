...
function TopVoted({ missions, maxVotes }) {
	...
	function resetAllVotes() {
		if (confirm("Are you sure you want to reset ALL votes?")) {
			setIsLoadingVote(true);
			axios
				.post(`/api/reforger-missions/reset_all_votes`)
				.then((response) => {
	...
	function resetMyVotes(missions) {
		if (confirm("Are you sure you want to reset your votes?")) {
			setIsLoadingVote(true);
			axios
				.post(`/api/reforger-missions/reset_my_votes`)
				.then((response) => {
					for (const mission of missions) {
						mission["hasVoted"] = false;
						const index = mission["votes"].indexOf(session.user["discord_id"], 0);
                                ...
                                <div className="flex flex-col items-start justify-between sm:flex-row">
                                    <div>
                                        <h2 style={{ margin: 0 }}>
                                            ...
                                            <Link
                                                href={`/reforger-missions/${mission.uniqueName}`}
...
export async function getServerSideProps(context) {
	...
	const missions = await (await MyMongo).db("prod").collection("reforger_missions")
		.aggregate([
			{
				$lookup: {
