...
export async function getServerSideProps() {
	const basePath = process.env.MISSION_REF_PATH;

	if (!basePath) {
		console.error("Environment variable MISSION_REF_PATH is not defined.");
		return { props: { missions: [] } };
	}

	const directoryPath = path.join(basePath, 'missions');
	let missionPathList: string[] = [];

	function fromDir(startPath: string, filter: string) {
		if (!fs.existsSync(startPath)) {
			console.warn("Directory does not exist:", startPath);
			return;
		}
		...
		for (const file of files) {
			...
			try {
				...
				console.error(`Error getting file stats: ${fullPath}`, err);
				continue;
			}

			if (stat.isDirectory()) {
				fromDir(fullPath, filter); // Recurse into subdirectory
			} else if (fullPath.endsWith(filter) && (!fullPath.includes("Configs"))) {
				missionPathList.push(fullPath)
			}
		}
	}

	fromDir(basePath, '.conf');

	let missions: any[] = [];

	for (const missionFilePath of missionPathList) {
		let mission = {
			name: "",
			author: "",
			description: "",
		...
		let confContent = '';
		try {
			confContent = fs.readFileSync(missionFilePath, 'utf-8');
		} catch (err) {
			console.error(`Error reading config file: ${missionFilePath}`, err);
			continue;
		}

		const getMatch = (regex: RegExp, content: string, label: string) => {
			const match = regex.exec(content);
			if (!match || !match[1]) {
				console.warn(`Could not find ${label} in file: ${missionFilePath}`);
				return '';
			}
			return match[1];
		};

		const entFilePath = getMatch(/\s[Ww]orld\s".*}([Ww]orlds.*)"/, confContent, 'entFilePath');
		const name = getMatch(/\sm_sName\s"(.*)"/, confContent, 'name');
		const author = getMatch(/\sm_sAuthor\s"(.*)"/, confContent, 'author');
		const description = getMatch(/\sm_sDescription\s"(.*)"/, confContent, 'description');
		let maxPlayerCount = +(getMatch(/\sm_iPlayerCount\s([0-9]+)/, confContent, 'maxPlayerCount'));
		let minPlayerCount = 0;

		let entContent = '';
		let world = '';
		let gameMode = 'No gamemode found'

		if (entFilePath) {
			const fullEntPath = path.join(basePath, entFilePath);
			try {
				entContent = fs.readFileSync(fullEntPath, 'utf-8');
				world = getMatch(/.*[Ww]orlds\/(.*).ent/, entContent, 'world');
			} catch (err) {
				console.error(`Error reading entity file: ${fullEntPath}`, err);
			}
		}
		...
		if (name) {
			let nameMatches = /(TVT|COOP|COTVT|LOL)\s\(([0-9]+)-([0-9]+)\)/.exec(name)
			if (nameMatches && nameMatches.length > 3) {
				if (["TVT","COOP","COTVT","LOL"].includes(nameMatches[1])) {
					gameMode = nameMatches[1]
				} else {
					gameMode = getMatch(/\sm_sGameMode\s"(.*)"/, confContent, 'gameMode');
				}
				minPlayerCount = +nameMatches[2]
				maxPlayerCount = +nameMatches[3]
			}		
		}

		mission.name = name || "No name found";
		mission.author = author || "No author found";
		mission.description = description || "No description found";
		mission.gameMode = gameMode || "No gameMode found";
		mission.maxPlayerCount = maxPlayerCount || 0;
		mission.minPlayerCount = minPlayerCount || 0;
		mission.world = world || "No world found";
